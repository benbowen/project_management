...
@app.route("/api/projects/<project_id>/generate-claude-md", methods=["POST"])
def generate_claude_md(project_id):
    project = load_project(project_id)
    if not project:
        return jsonify({"error": "not found"}), 404
    ...
    if not repo:
        return jsonify({"error": "No repo path set for this project. Edit the project and add one."}), 400

    repo_path = os.path.expanduser(repo)
    if not os.path.isdir(repo_path):
        return jsonify({"error": f"Repo directory not found: {repo_path}"}), 400
    ...
    def fmt_cards(cards):
        ...
        for c in cards:
            ...
            if c.get("tags"):
                lines.append(f"_Tags: {', '.join(c['tags'])}_")
            lines.append("")
        return "\n".join(lines)

    today = datetime.now().isoformat()[:10]
    in_progress = project["columns"].get("in_progress", [])
    next_cards = project["columns"].get("next", [])
    log_entries = (project.get("session_log") or [])[:3]

    log_section = ""
    for e in log_entries:
        log_section += f"- **{e['date']}**: {e['summary']}\n"
    if not log_section:
        log_section = "_No entries yet._\n"

    content = f"""# {project['name']} — Claude Context

> Auto-generated by Project Board on {today}.
...
**Repo:** {repo}
**Last updated:** {project.get('last_updated', today)}

## Currently In Progress

{fmt_cards(in_progress)}
## Up Next

{fmt_cards(next_cards[:5])}
## Notes

{project.get('notes', '_No notes._')}

## Recent Session Log

{log_section}"""

    out_path = os.path.join(repo_path, "CLAUDE.md")
    with open(out_path, "w") as f:
        f.write(content)

    return jsonify({"written_to": out_path})
